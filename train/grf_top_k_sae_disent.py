...
GRF_OUTPUT_DIR = os.path.join(MODEL_DIR, 'grfs_disent')
...
def load_model(model_path, input_dim, encoding_dim, sparsity_param):
    """Load the trained TiedGaitSAE model with disentanglement loss"""
...
def convert_to_sparse_basis(model, data):
    ...
    with torch.no_grad():
        _, activations = model.forward(torch.FloatTensor(data))
...
def plot_top_k_grfs(model, data, sparse_basis, feature_idx, k, output_dir):
    ...
    for i, idx in enumerate(top_k_indices):
        ...
        # Reconstruct the feature itself
        feature_vector = torch.zeros(1, sparse_basis.shape[1], dtype=torch.float32)  # Ensure the tensor is of type float32
        feature_vector[0, feature_idx] = float(sparse_basis[idx, feature_idx])  # Explicitly cast to float
        ...
        plt.plot(reconstructed_feature, label='Reconstructed Feature', linestyle=':')
...
def main():
    ...
    for feature_idx in range(encoding_dim):
